...
#col1 <- c(expression(hat(beta)[0]),expression(hat(beta)[1]),expression(hat(beta)[2]),expression(hat(beta)[3]),expression(hat(beta)[4]))
col2 <- c(fit.full.cl$alpha,fit.full.cl$coef.lin)
col3 <- c(fit.rob$alpha,fit.rob$coef.lin)

tabla.b1 <- cbind(col2,col3)
#tabla.b1 <- as.data.frame(tabla.b1)
#tabla.b1 <- cbind(c(as.character(as.expression(col1))),tabla.b1)
row.names(tabla.b1) <- c('$\\lambda$', "$\\mu$", "$\\pi$","$\\beta$","$\\beta_1$")
names(tabla.b1) <- c("Classical","Robust")
library(knitr)
...
```{r cowplot, echo=FALSE, warning=FALSE}
library(cowplot)
```
