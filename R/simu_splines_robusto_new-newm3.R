...
simulacion<-function(ejemplo, NITER,anterior,tipo.cont,n,desvio.epsilon,epsilon,degree.spline,typePhi){
 ...
 if(typePhi=="Huber"){
      nombreRob<-paste("PLAM_splines_new_Huber_ej", ejemplo,"_C",tipo.cont,"desde_",inicio,"hasta_",final,"_newm3.txt",sep="")
      nombre.g1<-paste("PLAM_splines_new_Huber_ej", ejemplo,"_C",tipo.cont,"desde_",inicio,"hasta_",final,"_g1_newm3.txt",sep="")
      nombre.g12-paste("PLAM_splines_new_Huber_ej", ejemplo,"_C",tipo.cont,"desde_",inicio,"hasta_",final,"_g2_newm3.txt",sep="")
    }
  if(typePhi=="Tukey"){
      nombreRob<-paste("PLAM_splines_new_Tukey_ej", ejemplo,"_C",tipo.cont,"desde_",inicio,"hasta_",final,"_newm3.txt",sep="")
      nombre.g1<-paste("PLAM_splines_new_Tukey_ej", ejemplo,"_C",tipo.cont,"desde_",inicio,"hasta_",final,"_g1_newm3.txt",sep="")
      nombre.g2<-paste("PLAM_splines_new_Tukey_ej", ejemplo,"_C",tipo.cont,"desde_",inicio,"hasta_",final,"_g2_newm3.txt",sep="")
  }
  ...
  if(class(AUX)!= 'try-error'){ 
    ...
    sal <- plam.rob(Y.cont,X, XT, np.point=grilla.np, maxit=20, method="MM")
    vv <- sal$vv
    alpha <- sal$alpha
    ...
    tukey.res <- sum(tukey.loss( residuos/desvio.hat ),na.rm=T)
    huber.res <- sum(huber.loss( residuos/desvio.hat ),na.rm=T)
    ...
    ecm.g2.rob[iter] <- mean( (g2.hat.grid - funcion.g2(grilla.np[,2]))^2 ,na.rm=T)
    ...
    guardo <-c(iter,coef.rob[iter,],ecm.g1.rob[iter], ecm.g2.rob[iter], ecm.global.rob[iter],
               ...
               degree.spline, nknots, huber.res, tukey.res , desvio.hat, nbasis, vv, alpha)
    ...
    guardo.g1 <-c(iter, as.vector(g1.hat.grid))
    largo <- length(guardo.g1)
    write(t(guardo.g1) , file=nombre.g1, ncol=largo, append=T)
    
    #Guardo g2 en la grilla
    guardo.g2 <-c(iter, as.vector(g2.hat.grid))
    largo <- length(guardo.g2)
    write(t(guardo.g2) , file=nombre.g2, ncol=largo, append=T)
