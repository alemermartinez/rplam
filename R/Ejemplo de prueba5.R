...
fit <- spm(ozone.level ~ f(daggett.pressure.gradient)+f(inversion.base.height) +f(inversion.base.temp))
summary(fit)
par(mfrow=c(2,2))
plot(fit)

y <- ozone.level
Z <- cbind(inversion.base.temp)
X <- cbind(daggett.pressure.gradient, inversion.base.height)
